      ...
      <div className="space-y-4 py-6 flex flex-col items-center">
        ...
        {preview ? (
          <div className="flex flex-col items-center gap-4 w-full animate-scale-in">
            <div className="relative h-44 w-44 rounded-full border border-hairline-strong overflow-hidden bg-canvas-soft flex items-center justify-center">
          ...
          >
            ...
            <span className="font-sans font-medium text-ink">Upload a selfie</span>
            <span className="text-xs text-muted font-mono">Processed in memory, never stored</span>
          </button>
        )}

        {error && (
          <p className="text-xs text-error text-center max-w-xs font-mono">{error}</p>
        )}
      </div>
